...
passport.use(new GoogleStrategy({
  ...
  try {
    ...
    if (!usuario) {
      ...
      // Crea un nuevo usuario; el password puede quedar vacío, ya que no se usará para login local
      usuario = await Usuario.create({
        nombre: displayName,
        email: emails[0].value,
        password: '', // no se utiliza para OAuth
        idRol: 2,     // asigna, por ejemplo, el rol de "usuario"
      });
    }
    // Generar tokens directamente sin usar sesiones.
    ...
    // Se pasa al callback la información del usuario junto con los tokens generados
